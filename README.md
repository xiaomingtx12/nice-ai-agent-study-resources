...
真正在循环的是应用拆解和方法复盘两栏——拆项目、抽技巧、把技巧沉成能带到下一次的工程判断。资源导航不在这个环里，是拆解和复盘卡住时随时去查的参考底座。

> skill 回喂那一步现在还没落地（skill 未封、工作流未搭），站点写的是要长成的样子，不是已完成的宣称。
...
- **方法与复盘**（`docs/notes/`）：从拆解里提炼能带到下一次的判断、模式和 agent 工程技巧，目标封成 skill 回喂下一轮。
- **共建与模板**（`docs/templates/`）：三栏的理念、迭代方向和写法约定。
