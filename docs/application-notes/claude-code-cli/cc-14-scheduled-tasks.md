---
description: "定时任务把 Agent Loop 从事件驱动扩展为时间驱动，调度器作为侧边车在空闲时刻注入伪用户消息。本章拆 5-field cron 解析、recurring 与 one-shot 相反的抖动方向、跨进程锁防重复触发、7 天 TTL 兜底被遗忘任务，以及 idle 检测如何避免抢占正在推理的 token。"
---
