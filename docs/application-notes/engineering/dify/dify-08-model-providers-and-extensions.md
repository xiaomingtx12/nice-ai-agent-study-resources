---
description: Dify v1.15.0 把所有模型提供商迁成运行在 Plugin Daemon 独立进程的插件，靠 graphon 抽象基类统一 invoke 接口、RSA 加密凭据、Redis 缓存 schema 和负载均衡。本章拆插件化模型运行时与沙箱隔离，看 30 多家厂商怎么在同一套接口下跑又不污染主进程。
---
