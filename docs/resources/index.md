...
| 你的当前缺口 | 先看哪条 | 为什么不是别条 |
| --- | --- | --- |
| 我缺一张 Agent 主线地图 | [HelloAgents](./hello-agents/) | 因为它更适合搭模块全景，不会一上来把你拖进架构深水区 |
| 我想边做项目边学 Agent | [AI Agents From Zero](./ai-agents-from-zero/) | 因为它更像项目驱动路径，不像 `HelloAgents` 那么偏地图 |
| 我想从 AI 编程入口真正做出东西 | [Easy-Vibe](./easy-vibe/) | 因为它在设计“怎么把人带进来”，不是先拆底层系统 |
| 我想知道 coding agent 为什么能工作 | [Learn Claude Code](./learn-claude-code/) | 因为它先拆 harness，不像 `CCB` 那样先给你白皮书式结构 |
| 我想补生产架构、预算、安全、治理 | [AI Agent Book](./ai-agent-book/) | 因为它直接讨论这些问题，不像 `OpenClaw` 那样更依赖真实源码背景 |
| 我想拆成熟 coding agent 的产品级边界 | [Claude Code Architecture（CCB）](./claude-code-architecture/) | 因为它更像逆向架构白皮书，不像 `Learn Claude Code` 那样偏教学 |
| 我想看真实大型系统怎么把这些概念落地 | [OpenClaw 源码解析](./openclaw-book/) | 因为它面对的是完整系统，不再只是教学材料或架构总结 |
...
这一组主要解决“Claude Code、Codex、Cursor 这类东西为什么能工作”。  
